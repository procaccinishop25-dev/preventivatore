...
def page_toolbar(titolo, descrizione=None, cta_label=None, cta_icon=None, cta_page=None, cta_key=None):
    """Header di pagina condiviso: titolo+descrizione a sinistra, CTA primaria a destra.
    Se cta_page è passato, la CTA è un link a pagina; altrimenti è un bottone che ritorna True se premuto."""
...
def list_item(
    titolo, sottotitolo,
    stato_testo, stato_tipo,
    azione_primaria_label, key_primaria, azione_primaria_icon="arrow_forward",
    azione_secondaria_label=None, azione_secondaria_icon=None, key_secondaria=None,
    azioni_menu=None,
):
    """Riga di lista condivisa: Titolo/sottotitolo | Stato (bullet leggero) | Azione primaria + al massimo
    una secondaria sempre visibile + menu overflow '\u22ef' per le azioni meno frequenti/distruttive.
    ...
    with col_azioni:
        n_extra = (1 if azione_secondaria_label else 0) + (1 if azioni_menu else 0)
        rapporti = [3] + [1] * n_extra
        sotto_colonne = st.columns(rapporti)

        with sotto_colonne[0]:
